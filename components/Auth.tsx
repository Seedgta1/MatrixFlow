...
import { UserPlus, LogIn, Hexagon, Link, ArrowRight, Mail, Phone, Loader2, Lock, User as UserIcon } from 'lucide-react';
...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md glass-card p-8 rounded-3xl shadow-2xl animate-enter z-10">
        ...
        <div className="text-center mb-8 flex flex-col items-center">
          ...
          <div className="relative w-24 h-24 mb-6 animate-float">
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[0_0_15px_rgba(99,102,241,0.6)]">
              <defs>
          ...
          <h1 className="text-4xl font-bold text-white mb-2 tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-indigo-200 to-slate-400">MatrixFlow</h1>
          <p className="text-slate-400 font-light tracking-wide">Next Gen Network Management</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1 flex items-center gap-1">
              <UserIcon className="w-3 h-3" /> Username
            </label>
            <input
              type="text"
              required
              className="w-full glass-input rounded-xl px-4 py-3.5 text-white placeholder-slate-500 focus:outline-none"
          ...
          {!isLogin && (
            <>
              <div className="grid grid-cols-2 gap-4 animate-enter">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1 flex items-center gap-1"><Mail className="w-3 h-3" /> Email</label>
                  <input
                    type="email"
                    required
                    className="w-full glass-input rounded-xl px-4 py-3.5 text-white placeholder-slate-500 focus:outline-none"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1 flex items-center gap-1"><Phone className="w-3 h-3" /> Telefono</label>
                  <input
                    type="tel"
                    required
                    className="w-full glass-input rounded-xl px-4 py-3.5 text-white placeholder-slate-500 focus:outline-none"
              ...
              <div className="space-y-1 animate-enter">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider ml-1">Sponsor</label>
                <input
                  type="text"
                  required
                  className="w-full glass-input rounded-xl px-4 py-3.5 text-white placeholder-slate-500 focus:outline-none"
          ...
          {error && <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-300 text-sm text-center animate-enter">{error}</div>}
          {success && <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-300 text-sm text-center animate-enter">{success}</div>}

          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 glass-button text-white font-bold rounded-xl shadow-lg transition-all transform flex items-center justify-center gap-2 mt-6 disabled:opacity-70 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 text-center">
          <button
            ...
            className="text-slate-500 hover:text-white text-sm transition-colors font-medium flex items-center justify-center gap-1 mx-auto group"
          >
